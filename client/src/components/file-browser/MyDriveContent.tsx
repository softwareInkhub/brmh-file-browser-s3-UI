...
const MyDriveContent: React.FC<MyDriveContentProps> = ({
  ...
  // State for selected file in Column View
  const [selectedFile, setSelectedFile] = useState<TreeNode | undefined>(undefined);

  // Convert S3Object to TreeNode for ColumnView
  const handleColumnFileClick = (treeNode: TreeNode) => {
    setSelectedFile(treeNode);
  };

  const handleColumnFolderClick = (treeNode: TreeNode) => {
    // For folders, navigate to the folder path
    onNavigate(treeNode.key);
  };
  ...
  return (
    <div className="flex-1">
      {viewMode === "column" ? (
        <>
          {/* Unified Navigation Bar - Home action bar */}
          <UnifiedNavBar
            viewMode={viewMode}
            ...
            <ColumnViewPreview
              ...
              onDownload={() => {
                if (selectedFile) {
                  ...
                  onFileClick(s3Object);
                }
              }}
              onStar={() => {
                // Handle star action
                console.log('Star clicked for:', selectedFile?.key);
              }}
              onRename={() => {
                // Handle rename action
                console.log('Rename clicked for:', selectedFile?.key);
              }}
              onMove={() => {
                // Handle move action
                console.log('Move clicked for:', selectedFile?.key);
              }}
              onDelete={() => {
                // Handle delete action
                console.log('Delete clicked for:', selectedFile?.key);
              }}
              isStarred={false}
            />
          </div>
